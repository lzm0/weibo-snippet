...
(() => {
	...
	polling()
		.catch(error => error.message === 'finish' ? console.log('获取完成', friends) : console.warn('出错了', error.message))
})()
```
### 根据粉丝数量排序
```javascript
friends.sort((a, b) => b.followers_count - a.followers_count).map(x => {return {'name': x.name, 'count': x.followers_count}})
```
